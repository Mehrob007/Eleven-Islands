        ...
        <div className="componentModal">
          ...
          <div className="componentModalHeaderNav">
            <div className="navigation">
              <button onClick={() => setModalStateNav(false)}>
                <img src={iconClouseBlack} alt="iconClouseBlack" />
              </button>
            ...
            >
              ...
              <span style={{ fontSize: "24px" }}>
                {isOpen ? (
                  ...
                  <RiArrowDownSLine style={{ rotate: "-90deg" }} />
                )}
              </span>
            </div>
            <TransitionGroup style={{ height: "auto" }}>
              {isOpen && (
                <CSSTransition timeout={200} classNames="fade">
                  ...
                  >
                    <Link
                      onClick={() => setModalStateNav(false)}
                      to="/products/all"
                    >
                      Смотреть все
                    </Link>
                    {typeSelect?.length ? typeSelect?.map((el, i) => (
                      <Link
                        key={i}
                        onClick={() => setModalStateNav(false)}
                        to={`/products/${el?.categoryId}`}
                      >
                        {el?.name}
                      </Link>
                    )) : ""}
          ...
          >
            ...
            >
              <Link onClick={() => setModalStateNav(false)} to="/gelary">
                <h2 style={{ margin: 0 }} className="Cotalog-nav-phone">
                  LookBook
                </h2>
              </Link>
              {/* <span style={{ fontSize: '24px' }}>
              ...
              {isOpen && (
                <CSSTransition timeout={200} classNames="fade">
                  ...
                  >
                    ...
                    <Link
                      onClick={() => setModalStateNav(false)}
                      to="/products/Футболки"
                    >
                      Футболки
                    </Link>
                    <Link
                      onClick={() => setModalStateNav(false)}
                      to="/products/Леггинсы"
                    >
                      Леггинсы
                    </Link>
                    <Link
                      onClick={() => setModalStateNav(false)}
                      to="/products/Толстовки"
                    >
                      Толстовки
                    </Link>
                    <Link
                      onClick={() => setModalStateNav(false)}
                      to="/products/Спортивные брюки"
                    >
                      Спортивные брюки
                    </Link>
                    <Link
                      onClick={() => setModalStateNav(false)}
                      to="/products/Аксессуары"
                    >
                      Аксессуары
                    </Link>
                  </div>
